...
void UMainWidget::NativeConstruct()
{
	...
	txt_MapLoad->SetVisibility(ESlateVisibility::Hidden);
	...
	if (bOpened && OutFiles.Num() > 0)
	{
		...
		if (FFileHelper::LoadFileToArray(FileData, *SourceFilePath))
		{
			...
			if (bSaved)
			{
				...
				MapLoadBtn->SetVisibility(ESlateVisibility::Visible);
				LoadImage->SetVisibility(ESlateVisibility::Visible);
				txt_MapLoad->SetVisibility(ESlateVisibility::Visible);
			}
			else
			{
				IsSelect = false;
